class Finder
  attr_reader :entries, :sum_goal
  def initialize(entries:, sum_goal:)
    @entries = entries
    @sum_goal = sum_goal
  end
  ...
  def get
    ...
    entries.each_with_index do |entry, i|
      entries.slice(i, entries.length).each_with_index do |second_entry, j|
        if (entry + second_entry) == sum_goal
          result_part1 = entry * second_entry
        end
        entries.slice(i + j, entries.length).each do |third_entry|
          if (entry + second_entry + third_entry) == sum_goal
            result_part2 = entry * second_entry * third_entry
            break
          end
        end
      end
